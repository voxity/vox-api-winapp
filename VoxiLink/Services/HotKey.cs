...
namespace VoxiLink.Services
{
    ...
    {
        public ModifierKeys MK { get; set; }
        public Keys K { get; set; }
        public HotKeyException(ModifierKeys mK, Keys k)
            : base(string.Format("Hotkey {0} + {1} already in use", mK, k))
        {
            MK = mK;
            K = k;
        }
    ...
    public sealed class HotKey : IDisposable
    {
        ...
        public void RegisterHotKey()
        {
            ...
            if (!_isKeyRegistered)
            {
                throw new HotKeyException(KeyModifier, Key);
            }
        }
